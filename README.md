...
| 脚本 | Base URL | 说明 |
|------|----------|------|
| `setup-claude-lightos-459695.sh` | `https://api.459695.xyz` | Claude Code + dpsk，固定使用 459695 API |
...
## 2. setup-claude-lightos-459695.sh

配置 Claude Code，使用懒猫 LightOS 专用的 459695 API 地址和 dpsk 模型。

### 快速使用

```bash
curl -O https://raw.githubusercontent.com/zenenznze/setup-claude-codex/main/setup-claude-lightos-459695.sh
chmod +x setup-claude-lightos-459695.sh
./setup-claude-lightos-459695.sh
```

也可以直接传 key：

```bash
./setup-claude-lightos-459695.sh "你的apikey"
```
